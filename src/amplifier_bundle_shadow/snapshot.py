...
class SnapshotManager:
    ...
    async def create_snapshot(
        ...
        if has_uncommitted:
            commit_sha = await self._create_snapshot_with_changes(
    ...
    async def _create_simple_bundle(self, repo_path: Path, output_path: Path) -> None:
        ...
        if not refs:
            ...
            await self._run_git(
                repo_path, "bundle", "create", str(output_path), "--all"
            )
        else:
    ...
    async def _create_snapshot_with_changes(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Sync working tree: copy source files AND delete files not in source
            # This ensures deleted files are properly captured in the snapshot
            await self._sync_working_tree(repo_path, tmp_repo)
            ...
            await self._run_git(
                ...
                "--allow-empty",
                "-m",
                "Shadow snapshot: uncommitted changes",
                "--author",
                "Shadow <shadow@localhost>",
            )
    ...
    async def _sync_working_tree(self, source: Path, target: Path) -> None:
        """
        Synchronize working tree from source to target.

        This method ensures the target directory exactly matches the source:
        - Copies all files/dirs from source to target
        - DELETES files/dirs in target that don't exist in source

        This properly captures file deletions in snapshots, which was
        previously a limitation (deleted files would still appear).
        """
        # Get sets of items (excluding .git)
        source_items = {item.name for item in source.iterdir() if item.name != ".git"}
        target_items = {item.name for item in target.iterdir() if item.name != ".git"}

        # Delete items in target that don't exist in source (captures deletions)
        for item_name in target_items - source_items:
        ...
        # Copy/update items from source to target
